...
import { Trash2, Mail, Save, AlertCircle, CheckCircle, X } from "lucide-react"
      ...
      <div className="max-w-7xl mx-auto">
            ...
            <div className="flex-1">
              ...
              {editingEmail ? (
                ...
                <div className="px-4 py-2 bg-gray-50 rounded-lg border border-gray-200">
                  <p className="text-gray-900 font-medium">{notificationEmail || "No configurado"}</p>
                </div>
              )}
            </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
          <div className="p-6 border-b border-gray-200">
            <h2 className="text-xl font-semibold text-gray-900">Mensajes Recibidos ({contacts.length})</h2>
          </div>
                  ...
                  {contacts.map((contact) => (
                    <React.Fragment key={contact.id}>
                      <tr className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-gray-600">{formatDate(contact.created_at)}</div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <button
                      ...
                      {/* Expanded row with query */}
                      <tr className="bg-gray-50">
                        <td colSpan="6" className="px-6 py-4">
                          <div className="text-sm">
                            <p className="font-medium text-gray-700 mb-2">Consulta:</p>
                            <p className="text-gray-600 whitespace-pre-wrap bg-white p-4 rounded border border-gray-200">
                              {contact.query}
                            </p>
                            <div className="mt-3 flex gap-4 text-xs text-gray-500">
                              <span>Términos aceptados: {contact.terms_accepted ? "✓ Sí" : "✗ No"}</span>
                              <span>Marketing aceptado: {contact.marketing_accepted ? "✓ Sí" : "✗ No"}</span>
                            </div>
                          </div>
                        </td>
                      </tr>
        ...
        {deleteConfirm && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
              ...
              <p className="text-gray-700 mb-6">¿Estás seguro de que deseas eliminar este mensaje de contacto?</p>
